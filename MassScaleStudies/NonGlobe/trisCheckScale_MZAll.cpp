// g++ -Wall -o trisCheckScale_MZAll `root-config --cflags --glibs` ../Utils/setTDRStyle.cc ../Utils/ntupleUtils.cc ../Utils/stabilityUtils.cc ../Utils/ConvoluteTemplate.cc ../Utils/histoFunc.h ../Utils/TPileupReweighting.h trisCheckScale_MZAll.cpp
...
int main(int argc, char** argv)
{
  ...
  float DAevtsLOHI = 0;
  float other = 0;
  float DAevtsHIHI_EBEB = 0;
  float DAevtsHIHI_EBnoEB = 0;
  float DAevtsHIHI_noEBnoEB = 0;
  ...
  for(int nYear = 0; nYear < 2; ++nYear){
    if(nYear == 0) year = 2012;
    if(nYear == 1) year = 2011;
  ...
  for(int ientry = 0; ientry < ntu_DA -> GetEntries(); ientry++)
  {
	...
	else if(e3x31/scERaw1 < 0.94 && e3x32/scERaw2 < 0.94) ++DAevtsLOLO;
	else if(e3x31/scERaw1 >= 0.94 && e3x32/scERaw2 < 0.94) ++DAevtsLOHI;
	else if(e3x31/scERaw1 < 0.94 && e3x32/scERaw2 >= 0.94) ++DAevtsLOHI;
	else ++other;


	if(e3x31/scERaw1 >= 0.94 && e3x32/scERaw2 >= 0.94 && isEB2 == 1 && isEB1 == 1) ++DAevtsHIHI_EBEB;
	else if(e3x31/scERaw1 >= 0.94 && e3x32/scERaw2 >= 0.94 && (isEB2 == 1 || isEB1 == 1) ) ++DAevtsHIHI_EBnoEB;
	else if(e3x31/scERaw1 >= 0.94 && e3x32/scERaw2 >= 0.94 && (isEB2 == 0 && isEB1 == 0) ) ++DAevtsHIHI_noEBnoEB;

        run_DA.push_back(runId);

	Mass_DA.push_back(ele1ele2_scM * sqrt(scEneReg1/scEne1 * corrEtR9_1 * scEneReg2/scEne2 * corrEtR9_2));
// 	std::cout << "ele1ele2_scM_regression*sqrt(scEneReg1/scEne1 * scEneReg2/scEne2) =  " 
// 		  << ele1ele2_scM_regression*sqrt(scEneReg1/scEne1 * scEneReg2/scEne2) << std::endl;
	...
	//	sct_reg_DA.push_back((scEneReg1*Rt1) + (scEneReg2*Rt2));
  ...
  for(int ientry = 0; ientry < ntu_MC -> GetEntries(); ientry++)
  {
	...
	if(std::string(EBEE) == "notEBEB" ){
	  ...
	  if(isEB2 == 1 && year == 2011 && R9_ele2 < 0.94)       energySmearing2 = gRandom->Gaus(1.,0.0008);
	  //EE                                                                                              
	  if(year == 2011 && isEB1 == 0 )       energySmearing1 = gRandom->Gaus(1.,0.055);
	  if(year == 2011 && isEB2 == 0 )       energySmearing2 = gRandom->Gaus(1.,0.055);
	  if(year == 2012 && isEB1 == 0 )       energySmearing1 = gRandom->Gaus(1.,0.04);
	  if(year == 2012 && isEB2 == 0 )       energySmearing2 = gRandom->Gaus(1.,0.04);
	  if(year == 2011 && isEB1 == 0 && R9_ele1 > 0.94)       energySmearing1 = gRandom->Gaus(1.,0.06);
	  if(year == 2011 && isEB2 == 0 && R9_ele2 > 0.94)       energySmearing2 = gRandom->Gaus(1.,0.06);
	  if(year == 2012 && isEB1 == 0 && R9_ele1 > 0.94)       energySmearing1 = gRandom->Gaus(1.,0.04);
	  if(year == 2012 && isEB2 == 0 && R9_ele2 > 0.94)       energySmearing2 = gRandom->Gaus(1.,0.04);
	}

	if(std::string(EBEE) == "notEBEB" && std::string(LOWHIGH) == "HIGH"){
	  if(isEB1 == 1 )       energySmearing1 = gRandom->Gaus(1.,0.0007);
	  if(isEB2 == 1 )       energySmearing2 = gRandom->Gaus(1.,0.0007);
	  if(isEB1 == 1 && year == 2012 && R9_ele1 > 0.94)       energySmearing1 = gRandom->Gaus(1.,0.00075);
	  if(isEB2 == 1 && year == 2012 && R9_ele2 > 0.94)       energySmearing2 = gRandom->Gaus(1.,0.00075);
	  if(isEB1 == 1 && year == 2011 && R9_ele1 < 0.94)       energySmearing1 = gRandom->Gaus(1.,0.0008);
	  if(isEB2 == 1 && year == 2011 && R9_ele2 < 0.94)       energySmearing2 = gRandom->Gaus(1.,0.0008);
	  //EE                                                                                              
	  if(year == 2011 && isEB1 == 0 )       energySmearing1 = gRandom->Gaus(1.,0.05);
	  if(year == 2011 && isEB2 == 0 )       energySmearing2 = gRandom->Gaus(1.,0.05);
	  if(year == 2012 && isEB1 == 0 )       energySmearing1 = gRandom->Gaus(1.,0.034);
	  if(year == 2012 && isEB2 == 0 )       energySmearing2 = gRandom->Gaus(1.,0.034);
	  if(year == 2011 && isEB1 == 0 && R9_ele1 > 0.94)       energySmearing1 = gRandom->Gaus(1.,0.055);
	  if(year == 2011 && isEB2 == 0 && R9_ele2 > 0.94)       energySmearing2 = gRandom->Gaus(1.,0.055);
	  if(year == 2012 && isEB1 == 0 && R9_ele1 > 0.94)       energySmearing1 = gRandom->Gaus(1.,0.04);
	  if(year == 2012 && isEB2 == 0 && R9_ele2 > 0.94)       energySmearing2 = gRandom->Gaus(1.,0.04);
	}
	...
	if(useMomentum) {energySmearing2 = 1.; energySmearing1 = 1.;	}

	if(scEneReg1/scEne1*scEt1*energySmearing1 < 25. || scEneReg2/scEne2*scEt2*energySmearing2 < 25.) continue;


	if(year == 2011)	ww = puReweighting->GetWeight((int)npu);
        puRe.push_back(ww);
  ...
  } /// fine dei 2 loop sugli annni
   std::cout << "1   totDAevts = " << totDAevts << std::endl;
   std::cout << "1   DAevtsHIHI = " << DAevtsHIHI << std::endl;
   std::cout << "1   DAevtsLOLO = " << DAevtsLOLO << std::endl;
   std::cout << "1   DAevtsLOHI = " << DAevtsLOHI << std::endl;
   std::cout << "1   other = " << other << std::endl;
   std::cout << "1   DAevtsHIHI_EBEB = " << DAevtsHIHI_EBEB << std::endl;
   std::cout << "1   DAevtsHIHI_EBnoEB = " << DAevtsHIHI_EBnoEB << std::endl;
   std::cout << "1   DAevtsHIHI_noEBnoEB = " << DAevtsHIHI_noEBnoEB << std::endl;
  ...
  for(int ientry = 0; ientry < nEntries; ++ientry)
  {
    ...
    if(std::string(EBEE) == "EB" && ( isEB1_DA.at(ientry) == 0 || isEB2_DA.at(ientry) == 0) ) continue;
    ...
    if(R91_DA.at(ientry) >= 0.94 && R92_DA.at(ientry) >= 0.94) {
     ...
     if(std::string(EBEE) == "EE" && (fabs(scEta1_DA.at(ientry)) < 1.566 || fabs(scEta1_DA.at(ientry)) > 2.5 )) continue; // endcap  
     if(std::string(EBEE) == "EE" && (fabs(scEta2_DA.at(ientry)) < 1.566 || fabs(scEta2_DA.at(ientry)) > 2.5 )) continue; // endcap  

      if (std::string(EBEE) == "EB" && (fabs(scEta1_DA.at(ientry)) > 1.4442 )) continue; // endcap                                      
      if (std::string(EBEE) == "EB" && (fabs(scEta2_DA.at(ientry)) > 1.4442 )) continue; // endcap                                      

      if(std::string(EBEE) == "notEBEB" && (isEB1_DA.at(ientry) == 1. && isEB2_DA.at(ientry) == 1.) ) continue; 
//      if(std::string(EBEE) == "EB" && isEB1_DA.at(ientry) == 0.) continue; 


      if (std::string(LOWHIGH) == "LOW" && (R91_DA.at(ientry) >= 0.94 || R92_DA.at(ientry) >= 0.94) ) continue;
      if (std::string(LOWHIGH) == "HIGH" && (R91_DA.at(ientry) < 0.94 || R92_DA.at(ientry) < 0.94) ) continue;
      if (std::string(LOWHIGH) == "PLUTO" && R91_DA.at(ientry) < 0.94 && R92_DA.at(ientry) < 0.94 ) continue;
...
//        std::cout << " sortedEntries.at(iSaved).laserCorr = " << sortedEntries.at(iSaved+1).laserCorr << std::endl;
  ...
  TH1F* h_Mass_allDA = new TH1F("h_Mass_allDA", "", 1000, 0.5, 1.5);
  TH1F* h_Mass_allMC = new TH1F("h_Mass_allMC", "", 1000, 0.5, 1.5);
...
//   TH1F* h_scRaw_DA = new TH1F("h_scRaw_DA", "", 4000, 0., 200.);
  ...
  for(int i = 0; i < nBins; ++i)
  {
    ...
    h_EoP_DA[i] = new TH1F(histoName, histoName, 1000, 0.5, 1.5);
    ...
    h_EoP_MC[i] = new TH1F(histoName, histoName, 1000, 0.5, 1.5);
    ...
    h_Et[i] = new TH1F(histoName, histoName, 5000, 0, 1000);
    h_Et[i]->SetLineColor(kRed+2);

    sprintf(histoName, "Et_MC_%d", i);
    h_Et_MC[i] = new TH1F(histoName, histoName, 5000, 0, 1000);
  ...
  for(unsigned int ientry = 0; ientry < DAEntries; ++ientry)
    ...
    h_Et[bin]->Fill(Ht_DA.at(ientry) );
    h_Et_allDA->Fill(Ht_DA.at(ientry) );
...
// 	  std::cout << " >>> EtBinEdge.at(i) = " << EtBinEdge.at(EtBinEdge.size()-1) << std::endl;
  ...
  for(unsigned int ientry = 0; ientry < MCEntries; ++ientry)
    {   
      ...
      if(std::string(EBEE) == "EB" && ( isEB1_MC.at(ientry) == 0 || isEB2_MC.at(ientry) == 0) ) continue;

      if (std::string(EBEE) == "EE" && (fabs(scEta1_MC.at(ientry)) < 1.566 || fabs(scEta1_MC.at(ientry)) > 2.5 )) continue; // endcap 
      if (std::string(EBEE) == "EE" && (fabs(scEta2_MC.at(ientry)) < 1.566 || fabs(scEta2_MC.at(ientry)) > 2.5 )) continue; // endcap 
      if (std::string(EBEE) == "EB" && (fabs(scEta1_MC.at(ientry)) > 1.4442 )) continue; // endcap                                     
      if (std::string(EBEE) == "EB" && (fabs(scEta2_MC.at(ientry)) > 1.4442 )) continue; // endcap                                     

      if(std::string(EBEE) == "notEBEB" && (isEB1_MC.at(ientry) == 1. && isEB2_MC.at(ientry) == 1.) ) continue; 

      if (std::string(LOWHIGH) == "LOW" && (R91_MC.at(ientry) >= 0.94 || R92_MC.at(ientry) >= 0.94) ) continue;
      if (std::string(LOWHIGH) == "HIGH" && (R91_MC.at(ientry) < 0.94 || R92_MC.at(ientry) < 0.94) ) continue;
      if (std::string(LOWHIGH) == "PLUTO" && R91_MC.at(ientry) < 0.94 && R92_MC.at(ientry) < 0.94 ) continue;
	...
	{	  
	  ...
	  if( (bin != EtBinEdge.size()-1 && referenceEt_MC > EtBinEdge.at(bin) && referenceEt_MC < EtBinEdge.at(bin+1)) ||
	      (bin == EtBinEdge.size()-1 && referenceEt_MC > EtBinEdge.at(bin) ) )
	    {
		  ...
		  h_Et_MC[int(bin)]->Fill(referenceEt_MC, puRe.at(ientry) );
		  break;
	    }
	}
      
      h_Et_allMC->Fill(Ht_MC.at(ientry), puRe.at(ientry));
  ...
  for(int i = 0; i < nBins; ++i)
  {
    ...
    //    if(std::string(LOWHIGH) == "LOW"){
    h_EoP_DA[i]->Rebin(2);
    h_EoP_MC[i]->Rebin(2);
    //    }

//     if(std::string(LOWHIGH) == "HIGH" && std::string(EBEE) == "notEBEB"){
//      h_EoP_DA[i]->Rebin(5);
//      h_EoP_MC[i]->Rebin(5);
//     }
//     if(std::string(LOWHIGH) == "LOW" && std::string(EBEE) == "notEBEB" && useMomentum == true){
//      h_EoP_DA[i]->Rebin(5);
//      h_EoP_MC[i]->Rebin(5);
//     }
//     else if(std::string(LOWHIGH) != "HIGH" || std::string(EBEE) != "notEBEB"){
//       h_EoP_DA[i]->Rebin(4);
//       h_EoP_MC[i]->Rebin(4);
//     }
    h_EoP_MC[i]->Smooth(6);
...
//     }
    ...
    //f_EoP[i] = new TF1(funcName, templateHistoFunc, 0.8, 1.2, 3, "histoFunc");
...
//    f_EoP[i] = new TF1(funcName, templateHistoFunc, 0.8, 1.3, 3, "histoFunc");
  ...
  std::string plotFolderName = "PLOTS_MZAll";
  if(doVsEach == "true") plotFolderName = "PLOTS_MZAll"; 
...
//   h_Et_allDA->GetXaxis()->SetTitle("Ht ");
