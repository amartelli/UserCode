#include "HiggsAnalysis/WWTo2l2nu/plugins/WWUtils.h"
             ...
             double jetPtMin, double jetEtaMax) 
{
  ...
  std::pair<WWjetIt,WWjetIt> tagJets (begin,begin) ;
  double maxInvMass = 0. ;

  //PG find the tagging jets

  //PG first loop over jets
  for (WWjetIt firstJet = begin ; 
		 firstJet != end ; 
		 ++firstJet ) 
  {
    ...
    for (WWjetIt secondJet = firstJet + 1 ; 
        	       secondJet != end ; 
        	       ++secondJet ) {
