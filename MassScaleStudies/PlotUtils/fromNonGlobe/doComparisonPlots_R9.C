...
//          std::string plotDirOut = "2011vs2012_R9_Shcorr";
  ...
  for(int ii=0; ii<2; ++ii){
    ...
    std::string file2011 = plotDir+"/results_"+category+"_none_scE_reg_2011.root";
    std::string file2012 = plotDir+"/results_"+category+"_none_scE_reg_2012.root";
   ...
   TCanvas* cplot;
   if(ii == 0) cplot = new TCanvas("gplot_EB", "gplot",100,100,725,500);
   if(ii == 1) cplot = new TCanvas("gplot_EE", "gplot",100,100,725,500);
   cplot->cd();
   TPad *cLeft;
   if(ii == 0) cLeft = new TPad("pad_EB","pad_EB",0.00,0.00,1.00,1.00);
   if(ii == 1) cLeft = new TPad("pad_EE","pad_EE",0.00,0.00,1.00,1.00);
  ...
  // pad settings                                                                     
  TH1F *hPad = (TH1F*)gPad->DrawFrame(0.7,xmin,1.,xmax);
  ...
  TLegend *tspec = new TLegend(0.24,0.80,0.59,0.99);
  tspec->SetFillColor(0);
  tspec->SetTextFont(42);
  if(ii == 0){
  tspec->AddEntry(graph2012,"2012 EB ","PL");
  tspec->AddEntry(graph2011,"2011 EB ","PL");
  }
  if(ii == 1){
  tspec->AddEntry(graph2012,"2012 EE ","PL");
  tspec->AddEntry(graph2011,"2011 EE ","PL");
  }
  ...
  tspec->Draw("same");
  if(ii == 0)  cplot->Print( (plotDirOut+"/EoP_vs_R9_EB_"+category+".png").c_str(),".png");
  if(ii == 1)  cplot->Print( (plotDirOut+"/EoP_vs_R9_EE_"+category+".png").c_str(),".png");
  ...
  // pad settings                                                                     
  TH1F *hPad = (TH1F*)gPad->DrawFrame(20,-0.03,100,0.03);
