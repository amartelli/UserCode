...
std::cout << " Entering breitWigner_crystalBallLowFWHM " << std::endl;
...
void BinnedFitZPeak(const std::string& category, const int& rebin, TH1F* h_mZ_DATA, TH1F* h_mZ_MC,
  ...
  std::cout << " DATA+MC fitting category: " << category << " reg = " << energyType <<  std::endl;
  ...
  if(category == "EE-EE" || category == "EEp" || category == "EEm" || category=="EE_R9_g" || category=="EE_R9_l" || category=="EE_Eta_l" || category=="EE_Eta_g" ){
   ...
   do{
       ...
       fStatusDATA = rpDATA;
       std::cout << " DATA fStatus " << fStatusDATA << std::endl;
       } while(fStatusDATA!=0);
   ...
   do{
       ...
       fStatusMC = rpMC;
       std::cout << " MC fStatus " << fStatusMC << std::endl;
       } while(fStatusMC!=0);
...
if(category == "EB-EB" || category == "EBp" || category == "EBm" || category=="EB_R9_g" || category=="EB_Eta_l"|| category=="EB_Eta_g"){
   ...
   do{
       ...
       //       std::cout << " data rand = " << (*(rand)) << std::endl;
//        std::cout << " bw_cb_DATA par 0 = " << bw_cb_DATA->GetParameter(0) << std::endl;
//        std::cout << " bw_cb_DATA par 1 = " << bw_cb_DATA->GetParameter(1) << std::endl;
//        std::cout << " bw_cb_DATA par 2 = " << bw_cb_DATA->GetParameter(2) << std::endl;
//        std::cout << " bw_cb_DATA par 3 = " << bw_cb_DATA->GetParameter(3) << std::endl;
//        std::cout << " bw_cb_DATA par 4 = " << bw_cb_DATA->GetParameter(4) << std::endl;
//        std::cout << " bw_cb_DATA par 5 = " << bw_cb_DATA->GetParameter(5) << std::endl;
//        std::cout << " bw_cb_DATA par 6 = " << bw_cb_DATA->GetParameter(6) << std::endl;
//        std::cout << " bw_cb_DATA par 7 = " << bw_cb_DATA->GetParameter(7) << std::endl;
       rpDATA=h_mZ_DATA -> Fit(funcNameDATA.c_str(),"SR0EL");
       //rpDATA=h_mZ_DATA -> Fit(funcNameDATA.c_str(),"SMR0L+");
       fStatusDATA = rpDATA;
       } while(fStatusDATA!=0);

   bw_cb_DATA -> Draw("same"); 

   std::cout << " fStatusDATA = " << fStatusDATA  << std::endl; 
   ...
   do{
       ...
       //       std::cout << " mc rand = " << (*(rand)) << std::endl;
//        std::cout << " bw_cb_MC par 0 = " << bw_cb_MC->GetParameter(0) << std::endl;
//        std::cout << " bw_cb_MC par 1 = " << bw_cb_MC->GetParameter(1) << std::endl;
//        std::cout << " bw_cb_MC par 2 = " << bw_cb_MC->GetParameter(2) << std::endl;
//        std::cout << " bw_cb_MC par 3 = " << bw_cb_MC->GetParameter(3) << std::endl;
//        std::cout << " bw_cb_MC par 4 = " << bw_cb_MC->GetParameter(4) << std::endl;
//        std::cout << " bw_cb_MC par 5 = " << bw_cb_MC->GetParameter(5) << std::endl;
//        std::cout << " bw_cb_MC par 6 = " << bw_cb_MC->GetParameter(6) << std::endl;
//        std::cout << " bw_cb_MC par 7 = " << bw_cb_MC->GetParameter(7) << std::endl;
       rpMC=h_mZ_MC -> Fit(funcNameMC.c_str(),"SR0EWL+");
       //rpMC=h_mZ_MC -> Fit(funcNameMC.c_str(),"SMR0L+");
       fStatusMC = rpMC;
       } while(fStatusMC!=0);

   bw_cb_MC -> Draw("same"); 

   std::cout << " fStatusMC = " << fStatusMC  << std::endl; 
  ...
  c -> Print((h_mZ_DATA_Name+energyType+"_rerecoICHEP2012.png").c_str(),"png");
  c1 -> Print((h_mZ_MC_Name+energyType+"_MC_IC_v2.png").c_str(),"png");
...
void BinnedFitZPeak(const std::string& category, const int& rebin, TH1F* h_mZ_DATA,
  ...
  std::cout << " OnlyDATA fitting category: " << category << " reg = " << energyType <<  std::endl;
...
if(category == "EB-EB" || category == "EBp" || category == "EBm" || category=="EB_R9_g" || category=="EB_Eta_l" || category=="EB_Eta_g"){
  ...
  c -> Print((h_mZ_DATA_Name+energyType+"_DATA_DATA_rerecoICHEP2012_v2.png").c_str(),"png");

  delete c;
}


void BinnedFitZPeak(const std::string& category, const int& rebin, TH1F* h_mZ, const std::string& type, float reScale, 
                    int nPoints, const double &min,const double &max, std::string energyType ){
  ...
  // draw peaks
  std::cout << ">>>Zutils::DrawZPeak::draw peaks" << std::endl;
  std::string h_mZ_Name = h_mZ->GetName();
 
  TCanvas* c = new TCanvas(("c_mZ"+h_mZ_Name).c_str(),("mZ - "+h_mZ_Name).c_str(),0,0,600,600);
  c->cd();
  c->SetGridx();
  c->SetGridy();
  ...
  char axisTitle[50];
  sprintf(axisTitle,"events / %.2e GeV/c^{2}",h_mZ->GetBinWidth(1));
  h_mZ -> Rebin(rebin);
  if(type == "MC")  h_mZ->Scale(reScale);
  h_mZ->GetXaxis()->SetLabelSize(0.03);
  h_mZ->GetXaxis()->SetLabelFont(42);
  h_mZ->GetXaxis()->SetTitleSize(0.03);
  h_mZ->GetXaxis()->SetTitleOffset(1.5);
  h_mZ->GetXaxis()->SetTitle(("m(e^{+}e^{-}) - "+category).c_str());
  h_mZ->GetYaxis()->SetLabelSize(0.03);
  h_mZ->GetYaxis()->SetTitle(axisTitle);
  
  Color_t iStyle = kGreen+2;
  if(type == "MC") iStyle = kRed+1;

  h_mZ->SetLineWidth(2);
  h_mZ->SetLineColor(iStyle);
  h_mZ->SetMarkerColor(kBlack);
  h_mZ->SetMarkerStyle(20);
  h_mZ->SetMarkerSize(0.5);
  h_mZ->GetYaxis()->SetTitleOffset(1.5);
  h_mZ->Draw("P");
  gPad->Update();
   
  h_mZ->GetYaxis()->SetRangeUser(0.,1.05*h_mZ->GetMaximum());
  gPad->Update();
  ...
  if(category == "EE-EE" || category == "EEp" || category == "EEm" || category=="EE_R9_g" || category=="EE_R9_l" || 
   ...
   int fStatus;
   do{
         SetParameterFunctionEE_EE(bw_cb, rand);
         if(type != "MC") rp = h_mZ -> Fit(funcName.c_str(),"SR0EL");
	 else if(type == "MC") rp = h_mZ -> Fit(funcName.c_str(),"SR0EWL");
         fStatus = rp;
         }while(fStatus!=0);
...
if(category == "EB-EE" || category=="EB_R9_l" ){
   ...
   int fStatus;
   do{
         SetParameterFunctionEB_EE(bw_cb, rand);
         if(type != "MC") rp = h_mZ -> Fit(funcName.c_str(),"SR0EL");
	 else if(type == "MC") rp = h_mZ -> Fit(funcName.c_str(),"SR0EWL");
         fStatus = rp;
         }while(fStatus!=0);
...
if(category == "EB-EB" || category == "EBp" || category == "EBm" || category=="EB_R9_g" || category=="EB_Eta_l" || category=="EB_Eta_g"){
   ...
   int fStatus;
   do{
         SetParameterFunctionEB_EB(bw_cb, rand);
	 if(type != "MC") rp = h_mZ -> Fit(funcName.c_str(),"SR0EL");
	 else if(type == "MC") rp = h_mZ -> Fit(funcName.c_str(),"SR0EWL");
         fStatus = rp;
         }while(fStatus!=0);
  ...
  // ------------------
  // get the statistics
 TPaveStats* p_mZ = (TPaveStats*)(h_mZ->GetListOfFunctions()->FindObject("stats"));
 p_mZ->SetX1NDC(0.69);
 p_mZ->SetX2NDC(0.99);
 p_mZ->SetY1NDC(0.69);
 p_mZ->SetY2NDC(0.99);
 p_mZ->SetTextColor(iStyle);
 p_mZ->SetOptFit(0012);
 p_mZ->Draw("same");
...
void SetParameterFunctionEE_EE(TF1* bw_cb, TRandom3 * rand){
  std::cout << " SetParameterFunctionEE_EE " << std::endl;
   // Z invariant Mass
...
void SetParameterFunctionEB_EE(TF1* bw_cb, TRandom3 * rand){
  std::cout << " SetParameterFunctionEB_EE " << std::endl;
  // Z invariant Mass
  bw_cb -> SetParName(1,"M_{Z}"); 
  bw_cb -> FixParameter(1,91.18);
  // Z FWHM
...
void SetParameterFunctionEB_EB(TF1* bw_cb, TRandom3 * rand){
  std::cout << " SetParameterFunctionEB_EB " << std::endl;
  // Z invariant Mass
  bw_cb -> SetParName(1,"M_{Z}"); 
  bw_cb -> FixParameter(1,91.18);
  // Z FWHM
