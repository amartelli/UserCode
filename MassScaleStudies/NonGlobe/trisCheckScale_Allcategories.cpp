...
//  	if(isEB1 == 0 )       energySmearing1 = gRandom->Gaus(1.,0.05);
//  	if(isEB2 == 0 )       energySmearing2 = gRandom->Gaus(1.,0.05);
  ...
  for(int i = 0; i < nBins; ++i)
  {
    ...
    h_EoP_DA[i] = new TH1F(histoName, histoName, 600, 0., 3.);
    ...
    h_EoP_MC[i] = new TH1F(histoName, histoName, 600, 0., 3.);
  ...
  for(unsigned int ientry = 0; ientry < DAEntries; ++ientry)
    ...
    if(strcmp(ENE,"scE_reg")==0){
      ...
      if(doVsEach == "true"){
	h_Et[bin]->Fill(scEt_reg_DA.at(ientry) );
  ...
  for(unsigned int ientry = 0; ientry < MCEntries; ++ientry)
	  ...
	  (bin == EtBinEdge.size()-1 && referenceEt_MC > EtBinEdge.at(bin) ) )
	{
	  if(PU == 0)
		...
		if(doVsEach == "true")  h_Et_MC[int(bin)]->Fill(scEt_reg_MC.at(ientry),puRe.at(int(ientry/2)));
	      }
	      if(strcmp(ENE,"scE")==0){
	    ...
	    if(doVsEach == "false") h_Et_MC[int(bin)]->Fill(scEt_reg_MC.at(ientry), puRe.at(int(ientry/2)) );
	    break;
	  } 
	}
    }
    ...
    if(PU == 1){
      if(doVsEach == "true"){
      if(strcmp(ENE,"scE_reg")==0)     	h_Et_allMC->Fill(scEt_reg_MC.at(ientry), puRe.at(int(ientry/2)));
      ...
      else h_Et_allMC->Fill(scEt_reg_MC.at(ientry), puRe.at(int(ientry/2)));
